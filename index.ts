...
import { genWorldPath } from './pathgen'
...
if (argv._[0] === 'build') {
    ...
    var outputdir = cwd();
    if (argv.world) {
        outputdir = genWorldPath(argv.world)
    }

    console.log(`Building ${argv._[1]}...`)
    var file = fs.readFileSync(argv._[1], 'utf8').replace(/\r\n/g, '\n')
    console.log(`Lexing...`)
    var lexed = lex(file)
    console.log(`Parsing...`)
    ...
    var compiled = compile(parsed, {name: argv._[1].replace(/\.item/g, ''), namespace: argv._[1].replace(/\.item/g, '')}, outputdir)
    // console.log(parsed)
